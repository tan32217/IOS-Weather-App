            ...
            ZStack {
                ...
                VStack(spacing: 15) {
                    ...
                    CitySearchView(searchText: $searchText, onCitySelected: { name, state in
                            selectedCity = City(name: name, state: state) // Update the selected city
                             print("Selected City: \(name), State: \(state)")
                        ...
                        NavigationLink(
                            destination: selectedCity != nil ? AnyView(
                                CityWeatherView(
//                                    weatherVM: weatherVM,
                                    city: selectedCity?.name ?? "", // Pass the dynamic city
                                    state: selectedCity?.state ?? "" ,// Pass the dynamic state
                                    onDisappear: resetNavigationState
                                )
                               
//                                .environmentObject(weatherVM)
                            ) : AnyView(EmptyView()), // Fallback to an empty view
